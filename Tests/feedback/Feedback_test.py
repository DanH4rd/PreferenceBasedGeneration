...
class TestFilter:
    ...
    def test_transformer_cos_feedback(self):
        ...
        feedback = CosDistFeedback(target_image=target_image, th_min= 0.01, th_max = 0.75
                                   , device='cuda', gen_model=gen_model)
        
        action_pairs = ActionPairsData(
                    action_pairs=torch.stack([gen_model.sample_random_actions(N=5).actions, 
        ...
        assert (action_pairs.action_pairs.shape[0] == preference_data.preference_pairs.shape[0])
