...
class CosDistFeedback(AbsFeedbackSource):
    ...
    def __init__(
        ...
        inputs = self.processor(
            images=self.target_image, return_tensors="pt", do_rescale=False
        )
    ...
    def generate_feedback(
        ...
        action_data = action_pairs.view(
            action_pairs.shape[0] * action_pairs.shape[1], action_pairs.shape[2]
        )
        ...
        cos_pair_distances = (
            ...
            - torch.nn.functional.cosine_similarity(
                self.target_image_embed, cos_embeds, dim=1, eps=1e-6
            )
        ).detach()
        cos_pair_distances = cos_pair_distances.view(
            action_pairs.shape[0], action_pairs.shape[1]
        )
        ...
        preferences[cos_pair_diff < 0] = torch.tensor([1.0, 0.0]).to(
            cos_pair_diff.device
        )
        preferences[cos_pair_diff > 0] = torch.tensor([0.0, 1.0]).to(
            cos_pair_diff.device
        )
        preferences[torch.abs(cos_pair_diff) < self.th_min] = torch.tensor(
            [0.5, 0.5]
        ).to(cos_pair_diff.device)
        preferences[(cos_pair_distances > self.th_max).all(dim=1)] = torch.tensor(
            [0.0, 0.0]
        ).to(cos_pair_diff.device)
