...
class BestActionTracker(AbsPreferenceDataGenerator):
    ...
    def GeneratePreferenceData(self, data:ActionData, limit:int) -> tuple[ActionPairsData, PairPreferenceData]:
        ...
        best_action_impossible_candidates = torch.stack(best_action_impossible_candidates, dim=0).unique(dim=0)


        # filter actions from impossible candidates
        candidate_actions = torch.flatten(action_tensor, start_dim=0, end_dim=1).unique(dim=0)

        for impossible_candidate in best_action_impossible_candidates:

            possible_candidate_pos_table = ~((candidate_actions - impossible_candidate) < 1e-10).all(dim=1)
        ...
        for candidate in candidate_actions:
            action_pair_tensor = torch.stack([self.best_action, candidate], dim = 0).unsqueeze(0)
            action_pair_data = ActionPairsData(actions_pairs=action_pair_tensor)

            pref_pair_data = self.prefDataGen.feedbackSource.GenerateFeedback(action_pair_data)
            preference = pref_pair_data.y[0]
            # print(preference)

            if (preference == torch.tensor([0.,1.])).all():
                # print('change')
        ...
        for action in all_actions:
            ...
            additional_pref_pairs.append(torch.tensor([1., 0.]))

        additional_action_pairs = torch.stack(additional_action_pairs)
        additional_pref_pairs = torch.stack(additional_pref_pairs)\

        action_pairs_tensor = torch.concat([action_pairs_data.actions_pairs, additional_action_pairs], dim=0)
        action_prefs_tensor = torch.concat([preference_data.y, additional_pref_pairs], dim=0)
