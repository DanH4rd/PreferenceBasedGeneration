...
class GraphPreferenceDataGeneration(AbsPreferenceDataGenerator):
    ...
    def __init__(self, feedbackSource:AbsFeedbackSource, deduceAdditionalLinks:bool = False):
        """
            Params:
    ...
    def AddPreferenceToGraphs(self, prefG:nx.DiGraph, genG:nx.Graph, node1:int, node2:int, preference:PairPreferenceData):        
        """
        ...
        if pref_tensor.shape[0] != 1:
            raise Exception(f'Only one preference is expected for AddPreferenceToGraphs, found: {pref_tensor.shape[0]}')
    ...
    def FindEdgeToAsk(self, actions_tensor:torch.tensor, edge_list:list, prefGraph:nx.DiGraph, genGraph:nx.Graph):
        """
        ...
        for node1, node2 in edge_list:
            ...
            if not genGraph.has_edge(u=node1, v=node2):
                ...
                action_pair_data = ActionPairsData(actions_pairs=torch.stack([action1, action2], dim=1))

                preference_data = self.feedbackSource.GenerateFeedback(action_pair_data)

                self.AddPreferenceToGraphs(prefG=prefGraph, genG=genGraph, node1=node1,
                                                    node2=node2, preference=preference_data)
    ...
    def GeneratePreferenceData(self, data:ActionData, limit:int) -> tuple[ActionPairsData, PairPreferenceData]:
        ...
        while (ask_counter < limit) and (len(general_graph.edges) != max_graph_edges):
            ...
            if len(components) > 1:
                ...
                components_combinations = combinations(components_connect_order.tolist(), 2)
                ...
                for comp1_id, comp2_id in components_combinations:
                    ...
                    gained_feedback = self.FindEdgeToAsk(actions_tensor=actions_tensor, edge_list=product(comp1, comp2),
                                       prefGraph=preference_graph, genGraph=general_graph)
            ...
            if not gained_feedback:
                ...
                nodes = sorted(dict(preference_graph.degree).keys(), key= lambda x: preference_graph.degree[x])

                nodes_combinations = combinations(nodes, 2)

                gained_feedback = self.FindEdgeToAsk(actions_tensor=actions_tensor, edge_list=nodes_combinations,
                                       prefGraph=preference_graph, genGraph=general_graph)
        ...
        for a,b,m in all_edges:
            if m['label'] == 'zero':
                ...
                preference_pair_tensor_list.append(torch.tensor([0., 0.]))
            elif m['label'] == 'base':
                pass
            else:
                raise Exception(f'Unknown label: {m['label']}')
