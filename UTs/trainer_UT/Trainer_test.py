...
trainer = ptLightningTrainer(model=reward_model,
                             batch_size=2
                             )
...
action_data, pref_data = dataGenerator.GeneratePreferenceData(data=gen_model.GetNoise(5), limit = 20)
...
model = ptLightningLatentWrapper(reward_model=reward_model, action=action, loss_func_obj=rewardLoss)

trainer = ptLightningTrainer(model=model,
                             batch_size=2
                             )
