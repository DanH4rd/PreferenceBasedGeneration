...
if __name__ == '__main__':
    ...
    feedback_source = CosDistFeedback(target_image=Image.open('GenerativeModelsData\\StackGan2\\target_images\\000387.jpg'),
                                      th_min= 0.01, th_max = 0.75
                                   , device='cuda', gen_model=gen_model)
    ...
    pref_loss_logger = TensorboardScalarLogger(name='Loss/Preference ', writer=tensorboard_writer)
    action_loss_logger = TensorboardScalarLogger(name='Loss/Action Reward', writer=tensorboard_writer)
    destination_handle_image_logger = TensorboardImageLogger(name='Image/Destination Image', writer=tensorboard_writer)

    if (feedback_source.target_image != None):
        tensorboard_writer.add_image('Image/Target', pil_to_tensor(feedback_source.target_image), 0)
    ...
    max_action_filter = ScoreActionFilter(mode='max', key= lambda x: reward_model.get_stable_rewards(x), limit = 10)
    min_action_filter = ScoreActionFilter(mode='min', key= lambda x: reward_model.get_stable_rewards(x), limit = 10)
